...
PLTPATH = r'/home/samth/sw/plt/'

SHOW_OPEN = True

FONT_NAME = "Bitstream Vera Sans"
FONT_SIZE = 10
PIXELS_PER_SECOND = 1400
PIXELS_PER_LINE = 12
PLOT_WIDTH = 1400
TIME_SCALE_WIDTH = 20
...
# 22738 1195667698.299940 open("/lib/tls/i686/cmov/libc.so.6", O_RDONLY) = 3
open_regex = re.compile (r'^\d+ +(\d+\.\d+) +open\("' + PLTPATH + r'(.*)", .*')
open_timestamp_group = 1
open_file_group = 2
...
class SyscallParser:
    ...
    def add_line (self, str):
        ...
        m = open_regex.search (str)
        if (m and SHOW_OPEN):
            timestamp = float (m.group (open_timestamp_group))
            file = m.group (open_file_group)
            s = OpenMark(timestamp, file)
            self.syscalls.append (s)
